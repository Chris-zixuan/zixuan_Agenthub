...
- 用户脚本的输出和输出约定在 UserProperty.cs 中，用户的脚本逻辑在 UserSCript.cs 中。
- 首先读取这两个文件，确定输出和输出与已有的逻辑。再结合用户的请求进行下一步。
...
- 脚本不考虑性能优化，不考虑耗时，以逻辑简单清晰为最主要。
- 除非必要否则简单的逻辑都应当自己写方法实现或者调用 references 目录下的方法。
